...
Meter::Meter()
{
    ...
    meterTitle.setText("Output", dontSendNotification);
    meterTitle.setJustificationType(Justification::centred);
...
void Meter::paint(Graphics& g)
{
    ...
    //g.drawImage(bgMeter, getLocalBounds().toFloat());
...
void Meter::resized()
{
    ...
    mBox.items.add(FlexItem(meterTitle).withFlex(1).withMargin(comboMargin));
